...
const HomeScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    budgetService.setNavigationCallback((params) => {
      console.log(
        "📱 Received navigation callback from budget service:",
        params
      );
    ...
    const initializeWatcher = async () => {
      try {
        ...
        const stopFunction = await budgetService.startFileWatcher(
          (freshBudget) => {
            // Only update if component is still mounted
            if (!isComponentMounted) return;

            console.log(
              "📡 Received budget update from file watcher:",
              freshBudget
            );
            ...
            const hasActualChanges =
              ...
              Object.keys(budget.categories).length !==
                Object.keys(freshBudget.categories).length ||
              Object.entries(budget.categories).some(
                ([key, value]) => freshBudget.categories[key] !== value
              );
            ...
            if (freshBudget && hasActualChanges) {
              ...
              setRecommendations(
                formatRecommendations(freshBudget.recommendations)
              );
          ...
          30000
        ); // Increased to 30 seconds interval
    ...
    const unsubscribe = navigation.addListener("focus", () => {
      ...
      if (fromChatbot) {
        ...
        console.log(
          "📋 Coming from Questionnaire, budget created - forcing refresh..."
        );
  ...
  const loadBudget = async (forceRefresh = false) => {
    ...
    try {
      console.log(
        `🔄 Loading budget from server (forceRefresh: ${forceRefresh})...`
      );
  ...
  const formatRecommendations = (recommendations) => {
    if (Array.isArray(recommendations) && recommendations.length > 0) {
      return recommendations.map((rec, index) => ({
        ...
        description: rec,
        icon: rec.toLowerCase().includes("groceries")
          ? "food"
          : rec.toLowerCase().includes("wish list")
          ? "book"
          : rec.toLowerCase().includes("prime")
          ? "tag"
          : "star",
        action: rec,
    ...
    try {
      ...
      for (const [category, amount] of categories) {
        ...
        if (isNaN(numAmount) || numAmount < 0) {
          setAdjustmentError(
            `Invalid amount for ${category}. Please enter a positive number.`
          );
          hasError = true;
          break;
        }
      ...
      setAdjustmentError(
        "Failed to update budget on server. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  const testChatbotSync = async () => {
    try {
      ...
      window.alert(
        `Current server budget total: ₹${freshBudget.total.toLocaleString()}\nCategories: ${
          Object.keys(freshBudget.categories).length
        }`
      );
    } catch (error) {
      ...
      >
        <Animated.View style={[styles.headerContent, animatedStyle]}>
          <View style={styles.headerTop}>
            <View>
              <Text
                style={[styles.welcomeText, { color: currentTheme.headerText }]}
              >
                Welcome back! 👋
              </Text>
              <Text
                style={[styles.headerTitle, { color: currentTheme.headerText }]}
              >
                Budget Dashboard
              </Text>
            </View>
          ...
          {budget.total > 0 && (
            <View style={styles.quickStats}>
              ...
              >
                <Text
                  style={[styles.statValue, { color: currentTheme.headerText }]}
                >
                  ₹{budget.total.toLocaleString()}
                </Text>
                <Text
                  style={[styles.statLabel, { color: currentTheme.headerText }]}
                >
                  Total Budget
                </Text>
              </View>
        ...
        <Animated.View style={animatedStyle}>
          {loading ? (
            ...
            >
              <Card.Content style={styles.loadingContent}>
                ...
                <Text
                  style={[styles.loadingText, { color: currentTheme.text }]}
                >
            ...
            <>
              {budget.total > 0 ? (
                <>
                  ...
                  <CategoryPieChart
                    categories={budget.categories || {}}
                    key={`piechart-${lastSyncTime}-${budget.total}-${
                      Object.keys(budget.categories || {}).length
                    }`}
                    theme={currentTheme}
                  />
                ...
                >
                  <Card.Content style={styles.emptyStateContent}>
                    ...
                    >
                      Create your first budget to get started with smart
                      financial planning
                    </Text>
                  </Card.Content>
                </Card>
          ...
          <View style={styles.actionCardsContainer}>
            ...
            >
              ...
              <Text
                style={[styles.actionCardTitle, { color: currentTheme.text }]}
              >
                AI Assistant
              </Text>
              <Text
          ...
          >
            ...
            >
              {Object.entries(editingBudget).map(([category, amount]) => (
                ...
                >
                  <Text
                    style={[styles.categoryLabel, { color: currentTheme.text }]}
                  >
                    {category}
                  </Text>
                  <View
                  ...
                  >
                    ...
                    <TextInput
                      value={String(amount)}
                      onChangeText={(text) =>
                        handleCategoryAmountChange(category, text)
                      }
                      ...
                      placeholder="0"
                      placeholderTextColor={currentTheme.text + "88"} // subtle placeholder, optional
                    />
                  </View>
                </View>
              ))}
              ...
              >
                ...
                <Text
                  style={[styles.totalAmount, { color: currentTheme.success }]}
                >
                  ₹
                  {Object.values(editingBudget)
